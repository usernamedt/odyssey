...
typedef struct
{
	 ...
	 * function. In the last case amount of raw bytes read from buf is stored in
	 * *processed.
	 */
	ssize_t (*write)(ZpqStream *zs,
	                 void const *buf,
	                 size_t size,
	                 size_t *processed);
...
zstd_write(ZpqStream *zstream, void const *buf, size_t size, size_t *processed)
{
	...
	do {
	  ...
	  zs->tx.pos == 0 &&
	  (in_buf.pos < size ||
	   zs->tx_not_flushed)); /* repeat sending data until first partial write */
...
zlib_read(ZpqStream *zstream, void *buf, size_t size)
{
	...
	while (1) {
		// START of inflate-specific part
		                             ...
		                             then decompress it */
		{
			...
			if (zs->rx.avail_out != size) {
				// if we managed to read some data, then we're done
				return size - zs->rx.avail_out;
			}
		}

		if (zs->rx.avail_in == 0) {
			// if no data available in receiver buffer, reset the the buffer
			// pointer
			zs->rx.next_in = zs->rx_buf;
		}
		// END of inflate-specific part
		// READ
		rc = zs->rx_func(zs->arg,
		                 zs->rx.next_in + zs->rx.avail_in,
		                 zs->rx_buf + ZLIB_BUFFER_SIZE -
		                   (zs->rx.next_in + zs->rx.avail_in));
		if (rc > 0) {
			zs->rx.avail_in += rc;
		} else {
...
zlib_write(ZpqStream *zstream, void const *buf, size_t size, size_t *processed)
{
	...
	} while (zs->tx.avail_out == ZLIB_BUFFER_SIZE &&
	         zs->tx.avail_in !=
	           0); /* repeat sending data until first partial write */
...
ZpqStream *
zpq_create(zpq_tx_func tx_func,
           zpq_rx_func rx_func,
           void *arg,
           char compression_algorithm)
{
	int i;
	if (compression_algorithm == ZPQ_NO_COMPRESSION) {
		return NULL;
	}

	int zpq_algorithm_impl = -1;
	for (i = 0; zpq_algorithms[i].name != NULL; i++) {
		if (zpq_algorithms[i].name() == compression_algorithm) {
			zpq_algorithm_impl = i;
		}
	}

	if (zpq_algorithm_impl == -1) {
		return NULL;
	}

	ZpqStream *zs =
	  zpq_algorithms[zpq_algorithm_impl].create(tx_func, rx_func, arg);

	if (zs == NULL) {
		return NULL;
	}

	zs->zpq_algorithm_impl = zpq_algorithm_impl;
	return zs;
}
...
{
	return zpq_algorithms[zs->zpq_algorithm_impl].read(zs, buf, size);
}

ssize_t
zpq_write(ZpqStream *zs, void const *buf, size_t size, size_t *processed)
{
	return zpq_algorithms[zs->zpq_algorithm_impl].write(
	  zs, buf, size, processed);
}
...
{
	if (zs)
		zpq_algorithms[zs->zpq_algorithm_impl].free(zs);
}

char const *
zpq_error(ZpqStream *zs)
{
	return zpq_algorithms[zs->zpq_algorithm_impl].error(zs);
}
