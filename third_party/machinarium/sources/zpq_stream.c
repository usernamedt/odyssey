...
typedef struct
{
	 ...
	 */
	mm_zpq_stream *(*create)(mm_zpq_tx_func tx_func,
	 ...
	 * the rx function.
	 */
	ssize_t (*read)(mm_zpq_stream *zs, void *buf, size_t size);
	 ...
	 * *processed.
	 */
	ssize_t (*write)(mm_zpq_stream *zs,
	                 void const *buf,
	                 size_t size,
	                 size_t *processed);
            ...
            size_t rx_data_size)
{
	...
	return (mm_zpq_stream *)zs;
}

static ssize_t
zstd_read(mm_zpq_stream *zstream, void *buf, size_t size)
{
...
static ssize_t
zstd_write(mm_zpq_stream *zstream,
           void const *buf,
           size_t size,
           size_t *processed)
{
...
static void
zstd_free(mm_zpq_stream *zstream)
{
	ZstdStream *zs = (ZstdStream *)zstream;
	if (zs != NULL) {
...
static char const *
zstd_error(mm_zpq_stream *zstream)
{
	ZstdStream *zs = (ZstdStream *)zstream;
	return zs->rx_error;
}
...
static size_t
zstd_buffered_tx(mm_zpq_stream *zstream)
{
	ZstdStream *zs = (ZstdStream *)zstream;
...
static size_t
zstd_buffered_rx(mm_zpq_stream *zstream)
{
	ZstdStream *zs = (ZstdStream *)zstream;
...
static _Bool
zstd_deferred_rx(mm_zpq_stream *zstream)
{
	ZstdStream *zs = (ZstdStream *)zstream;
...
static ssize_t
zlib_read(mm_zpq_stream *zstream, void *buf, size_t size)
{
	...
	while (1) {
		...
		rc = zs->rx_func(zs->arg,
		                 ...
		                 zs->rx_buf + ZLIB_BUFFER_SIZE - zs->rx.next_in -
		                   zs->rx.avail_in);
        /* if we've made a call to rx function, reset the deferred rx flag */
        zs->deferred_rx_call = 0;
		if (rc > 0) {
...
static ssize_t
zlib_write(mm_zpq_stream *zstream,
           void const *buf,
           size_t size,
           size_t *processed)
{
	...
	do {
		...
		rc = zs->tx_func(
		  zs->arg, zs->tx.next_out, ZLIB_BUFFER_SIZE - zs->tx.avail_out);
		if (rc > 0) {
			zs->tx.next_out += rc;
			zs->tx.avail_out += rc;
		} else {
...
static void
zlib_free(mm_zpq_stream *zstream)
{
	ZlibStream *zs = (ZlibStream *)zstream;
	if (zs != NULL) {
		inflateEnd(&zs->rx);
...
static char const *
zlib_error(mm_zpq_stream *zstream)
{
	ZlibStream *zs = (ZlibStream *)zstream;
	return zs->rx.msg;
}
...
static size_t
zlib_buffered_tx(mm_zpq_stream *zstream)
{
	ZlibStream *zs = (ZlibStream *)zstream;
...
static size_t
zlib_buffered_rx(mm_zpq_stream *zstream)
{
	ZlibStream *zs = (ZlibStream *)zstream;
...
static _Bool
zlib_deferred_rx(mm_zpq_stream *zstream)
{
	ZlibStream *zs = (ZlibStream *)zstream;
	return zs != NULL ? zs->deferred_rx_call : 0;
}
...
zpq_create(int algorithm_impl,
           ...
           size_t rx_data_size)
{
	mm_zpq_stream *stream = zpq_algorithms[algorithm_impl].create(
